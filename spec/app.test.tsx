...
describe('Main Page Different Streaming Content Displays', () => {
  ...
  describe('trending streaming content metadata display', () => {
    ...
    describe('When carousel buttons are pressed', () => {
      ...
      describe('When right carousel button is pressed', () => {
        ...
        describe('When at the end of the right side of the carousel', () => {
          ...
          describe('When moving pressing left carousel button when at the end of the right side carousel', () => {
            beforeEach(async () => {
              // need to wait for the smooth effect to occur amd then buttons are updated to be enabled/disabled
              // carousel mock scroll value would still be 3000 even after scrollLeftButton is clicked
      ...
      describe('When left carousel button is pressed', () => {
        beforeEach(() => {
          act(() => {
            app.home.trending.scrollLeftButton.click();
          });
        ...
        it('should still have the left button disabled', async () => {
          await waitFor(async () => {
            expect(app.home.trending.scrollLeftButton.isDisabled).toBe(true);
          });
        });
        ...
        it('should not move', async () => {
          await act(async () => {
            expect(carouselRefMock.current.scrollLeft).toBe(0);
          });
        });

          describe('When left and right carousel button is pressed at beginning', () => {
            beforeEach(async () => {
              await waitFor(async () => {
                app.home.trending.scrollRightButton.click();
                app.home.trending.scrollRightButton.click();
            ...
            it('should scroll to the right once', async () => {
              await waitFor(async () => {
                expect(carouselRefMock.current.scrollLeft).toBe(600);
              });
            });

            it('should have the right button enabled', async () => {
              await waitFor(() => {
                expect(app.home.trending.scrollRightButton.isDisabled).toBe(false);
              });
            });

            it('should have the left button enabled', async () => {
              await waitFor(() => {
                expect(app.home.trending.scrollLeftButton.isDisabled).toBe(false);
              });
            });
